...
async function updatePullRequestDescription(pr: PullRequest, octokit, coverage: string) {
  const params = { pull_number: pr.issueNumber, owner: pr.owner, repo: pr.repo };
...
function buildPullRequestDescription(body: string, coverage: string) {
  ...
  body = body.substring(0, start) + coverage + body.substring(end + 2, body.length);
  return body;
}

async function getPullRequestFiles(octokit, pr: PullRequest) {
  const compareResp = await octokit.repos.compareCommits({ base: pr.base, head: pr.head, owner: pr.owner, repo: pr.repo });
...
function generateReport(path: string, files: string[]): Report {
  const data = fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/${path}`, 'utf8');
...
function calculateCoverage(changedFiles: string[], report: {[key: string] : Report}): Report {

  const output: Report[] = [];
  const reports = reportToArray(report);

  changedFiles.forEach(cf => {
    const item = reports.find(r => r.fileName.toLocaleLowerCase().endsWith(cf));
    if (item) {
...
function addCoverageReport(c1: CoverageReport, c2: CoverageReport): CoverageReport {
  const res = {
